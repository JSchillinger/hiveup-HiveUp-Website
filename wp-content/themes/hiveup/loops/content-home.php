...
?>
  ...
  <article role="article" id="post_<?php the_ID()?>">
    ...
    <section>
      ...
      <?php the_content( __( '&hellip; ' . __('Continue reading', 'hiveup' ) . ' <i class="fa fa-arrow-right"></i>', 'hiveup' ) ); ?>
    </section>
    <footer>
      <p class="text-muted" style="margin-bottom: 20px;">
        <i class="fa fa-folder-open-o"></i>&nbsp;
        <?php _e('Category', 'hiveup'); ?>:
        <?php the_category(', ') ?><br/>
        <i class="fa fa-comment-o"></i>&nbsp;
        <?php _e('Comments', 'hiveup'); ?>:
        <?php comments_popup_link(__('None', 'hiveup'), '1', '%'); ?>
      </p>
    </footer>
  </article>
  <?php endwhile; ?>
